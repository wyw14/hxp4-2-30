...
router.get('/games', (req, res) => {
  try {
    ...
    const data: GameListItem[] = games.map((g) => ({
      id: g.id,
      level: g.level,
      status: g.status,
      steps: g.steps,
...
router.get('/games/:id', (req, res) => {
  try {
    ...
    if (!game) {
      const response: ApiResponse<void> = { success: false, error: '游戏不存在' };
...
router.post('/games/:id/extend', (req, res) => {
  try {
    ...
    if (!coord || typeof coord.q !== 'number' || typeof coord.r !== 'number') {
      const response: ApiResponse<void> = { success: false, error: '坐标参数无效' };
    ...
    const response: ApiResponse<GameState> = {
      ...
      newlyConnected: result.newlyConnected,
    };
    res.json(response);
  } catch (error) {
    const response: ApiResponse<void> = {
...
router.post('/games/:id/reset', (req, res) => {
  try {
    ...
    const gameWithId: GameState = { ...newGame, id: game.id, createdAt: game.createdAt };
    saveGame(gameWithId);
...
router.post('/games/:id/find-path', (req, res) => {
  try {
    ...
    if (!from || !to) {
      const response: ApiResponse<void> = { success: false, error: '起点和终点都需要' };
      return res.status(400).json(response);
    }
